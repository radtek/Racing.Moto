app.controller('annoucementController', ['$scope', '$rootScope', '$http', '$compile', '$timeout', '$q', '$sce', function ($scope, $rootScope, $http, $compile, $timeout, $q, $sce) {
    ...
    $scope.news = {
        Data: null,
        init: function () {
            $http.post('/Admin/PK/GetNewsAnnouncement').then(function (res) {
                console.log(res);
                if (res.data.Success) {
        ...
        save: function () {
            ...
            $http.post('/Admin/PK/SaveNews', $scope.news.Data).then(function (res) {
                console.log(res);
                if (res.data.Success) {
                    alert('修改成功!');
                } else {
...
app.controller('marqueeController', ['$scope', '$rootScope', '$http', '$compile', '$timeout', '$q', '$sce', function ($scope, $rootScope, $http, $compile, $timeout, $q, $sce) {
    ...
    $scope.news = {
        Data: null,
        init: function () {
            $http.post('/Admin/PK/GetNewsMarquee').then(function (res) {
                console.log(res);
                if (res.data.Success) {
                    $scope.news.Data = res.data.Data;
